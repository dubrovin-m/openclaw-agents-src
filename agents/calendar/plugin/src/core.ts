...
export type ProviderLabel = { id: string; name: string; backgroundColor: string };
export type ParentCategory = { id: string; name: string };
...
const finitePct = (value: unknown): value is number =>
...
function label(value: unknown, path: string): ProviderLabel {
...
function targets(value: unknown, path: string): Record<string, number> {
  ...
  for (const [key, pct] of Object.entries(object)) {
    if (!nonEmpty(key) || !finitePct(pct)) throw new Error(`${path} contains an invalid target`);
    result[key] = pct;
  }
  return result;
}
...
export function parseCalendarConfig(value: unknown): CalendarConfig {
  ...
  const read = provider.read.map((item) => {
    if (!nonEmpty(item)) throw new Error("providerTools.read must contain non-empty tool names");
    return item.trim();
  });
  if (read.length < 1 || new Set(read).size !== read.length) throw new Error("providerTools.read must contain unique tool names");
  ...
  if (!Array.isArray(root.parents) || root.parents.length < 1) throw new Error("parents must be a non-empty array");
  const parents: ParentCategory[] = root.parents.map((item, index) => {
  ...
  if (new Set(parents.map((item) => item.id)).size !== parents.length) throw new Error("parent ids must be unique");

  if (!Array.isArray(root.leaves) || root.leaves.length < 1) throw new Error("leaves must be a non-empty array");
  const parentIds = new Set(parents.map((item) => item.id));
  const leaves: LeafCategory[] = root.leaves.map((item, index) => {
    const object = asRecord(item);
    if (!object || !nonEmpty(object.id) || !nonEmpty(object.name)
  ...
  if (!close(parentTotal, 100) || !close(leafTotal, 100)) throw new Error("management targets must sum to 100%");
...
function parseClock(clock: string) {
  ...
  if (!match) throw new Error("invalid clock");
  return [Number(match[1]), Number(match[2])] as const;
}
export function localDateTimeMs(date: string, clock: string) {
  if (!/^\d{4}-\d{2}-\d{2}$/u.test(date)) throw new Error("invalid local date");
  const [year, month, day] = date.split("-").map(Number);
  const [hour, minute] = parseClock(clock);
  const target = Date.UTC(year, month - 1, day, hour, minute, 0);
  let guess = target;
  for (let i = 0; i < 4; i += 1) guess = target - offsetAt(guess);
  return guess;
}
...
export function localDate(epochMs: number) {
  const p = zonedParts(epochMs);
...
export function previousWorkingDates(endDate: string, count: number) {
  const dates: string[] = [];
  let cursor = endDate;
  while (dates.length < count) {
    if (isWorkingDate(cursor)) dates.push(cursor);
    cursor = addCivilDays(cursor, -1);
  }
  return dates.reverse();
}

export function nextWorkingDate(fromDate: string) {
  let cursor = addCivilDays(fromDate, 1);
  while (!isWorkingDate(cursor)) cursor = addCivilDays(cursor, 1);
  return cursor;
}

export type ReviewKind = "daily" | "next_workday" | "biweekly";
export function reviewWindow(kind: ReviewKind, boundaryIso = new Date().toISOString()) {
  const boundaryMs = Date.parse(boundaryIso);
  if (!Number.isFinite(boundaryMs)) throw new Error("boundary must be a valid ISO timestamp");
...
export function analyzeCalendar(configValue: unknown, kind: ReviewKind, boundaryIso: string | undefined, eventsValue: unknown) {
  const config = parseCalendarConfig(configValue);
  const window = reviewWindow(kind, boundaryIso);
  if (!Array.isArray(eventsValue)) throw new Error("events must be an array");
  const leafMap = new Map(config.leaves.map((leaf) => [leaf.id, leaf]));
  let excludedAllDayEvents = 0;
  const events: Array<{id: string; start: number; end: number; classification: string | null}> = eventsValue.flatMap((item, index) => {
    const object = asRecord(item);
    ...
    if (classification !== null && !leafMap.has(classification)) throw new Error(`events[${index}] references an unknown classification`);
    ...
    if (!Number.isFinite(start) || !Number.isFinite(end) || end <= start) throw new Error(`events[${index}] has an invalid interval`);
    return [{ id: object.id.trim(), start, end, classification }];
  });

  const byLeafMs: Record<string, number> = Object.fromEntries(config.leaves.map((leaf) => [leaf.id, 0]));
  let totalLoadMs = 0;
  ...
  for (const date of window.workDates) {
    ...
    const points = new Set<number>([dayStart, dayEnd, baselineStart, baselineEnd]);
    ...
    for (let i = 0; i < boundaries.length - 1; i += 1) {
      ...
      if (active.length === 0) {
        if (inBaseline) freeBaselineMs += duration;
        continue;
      }
      totalLoadMs += duration;
      if (!inBaseline) outsideBaselineMs += duration;
  ...
  const byParentMs: Record<string, number> = Object.fromEntries(config.parents.map((parent) => [parent.id, 0]));
  for (const leaf of managementLeaves) byParentMs[leaf.parentId as string] += byLeafMs[leaf.id];
