...
export function parseCalendarConfig(value: unknown): CalendarConfig {
  ...
  const read = native.read.map((item) => {
    if (!nonEmpty(item)) throw new Error("nativeTools.read must contain non-empty tool names");
    return item.trim();
  });
  if (read.length < 1 || new Set(read).size !== read.length) throw new Error("nativeTools.read must contain unique tool names");
