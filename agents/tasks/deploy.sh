...
db_starting_eligible(){ local state; state=$(read_db_state) || return 1; node -e 'const s=JSON.parse(process.argv[1]),allowed=process.argv[2].split(/ +/).filter(Boolean).map(Number);if(!allowed.includes(s.user_version)||s.integrity!=="ok"||s.fk!==0||(s.user_version===4&&!s.physical_v4)||(s.user_version===5&&!s.physical_v5)||(s.user_version===6&&!s.physical_v6)||(s.user_version===7&&!s.physical_v7)||(s.user_version===8&&!s.physical_v8)||(s.user_version===9&&!s.physical_v9))process.exit(1)' "$state" "$FROM_SQLITE_SCHEMAS"; }
