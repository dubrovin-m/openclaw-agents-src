...
NODE
  ...
  contacts_inspect=$(oc plugins inspect contacts --runtime --json); node -e 'const x=JSON.parse(process.argv[1]),p=x?.plugin;if(p?.id!=="contacts"||p?.packageVersion!=="0.1.0"||p?.status!=="loaded"||p?.toolNames?.length!==9)process.exit(1)' "$contacts_inspect"
