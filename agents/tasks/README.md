...
When the frozen release declares `task_governance.kind = private-bootstrap-v1`, first activation from the declared predecessor also requires the owner-private bootstrap file at the release-declared state-relative path. That file contains only canonical `P-*` Office CEO member identities and one canonical `L-*` personal Label identity, remains outside Git/Nexus, must be mode `0600`, and is validated before any deployment mutation. Deployment hashes it during preflight, rejects changes before activation, creates or reuses the shared Person Group, writes stable Task-domain bindings, and validates the resulting authoritative Contacts/Tasks state before the Gateway restarts. Once the target runtime is exact, the bootstrap file is not authoritative and is not required for normal reads or no-op target verification.

The current frozen release describes only the supported steady state. When a future Task Agent release needs a bounded predecessor transition, that release may declare a new predecessor fingerprint explicitly; completed predecessor releases are retained in Git history rather than in the active tree. Component identities are independent inside that atomic release: a new release may reuse the predecessor Task plugin version and frozen artifact only when the predecessor verifier proves the plugin name, version, artifact path, and SHA-256 are unchanged. `taskctl`, workspace, schema, or other release state may still advance through the single deployment and rollback lifecycle without an artificial plugin version bump.
...
The initial clean-history public-source migration is the single exception to Git-history materialization of the already-qualified `0.4.16` predecessor. `public-source-bootstrap.json` pins that exception to the exact `0.4.17` release fingerprint, legacy predecessor revisions, initial public snapshot commit/tree, bootstrap parent, and the exact historical schema-v4/schema-v5 revisions used only by migration regressions. The release-predecessor verifier may use this bridge only while the declared release predecessor is absent from public history; once a release predecessor exists in public history, normal historical predecessor verification is mandatory. Historical private-only migration or updater/Doctor provenance fixtures may continue to be omitted only after the marker's pinned public snapshot and exact fixture revision are independently verified. Current deterministic Project, Recurrence, predecessor, deployment, and rollback regressions still run, and the marker must not authorize a current-release predecessor once that predecessor exists in public history.
