...
The initial clean-history public-source migration is the single exception to Git-history materialization of the already-qualified `0.4.16` predecessor. `public-source-bootstrap.json` pins that exception to the exact `0.4.17` release fingerprint, legacy predecessor revisions, initial public snapshot commit/tree, and bootstrap parent. The verifier may use this bridge only when both legacy predecessor commits are absent; partial history, marker drift, release drift, or snapshot/ancestry drift fails closed. The historical workspace-retirement fixture may skip only after this exact bridge is verified. The bridge is transition-only and must be removed once the next release has a predecessor in public repository history.
