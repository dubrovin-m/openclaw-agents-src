...
The initial clean-history public-source migration is the single exception to Git-history materialization of the already-qualified `0.4.16` predecessor. `public-source-bootstrap.json` pins that exception to the exact `0.4.17` release fingerprint, legacy predecessor revisions, initial public snapshot commit/tree, bootstrap parent, and the exact historical schema-v4 revision used only by the Batch 7 migration regression. The verifier may use this bridge only when both legacy predecessor commits are absent; partial history, marker drift, release drift, or snapshot/ancestry drift fails closed. The historical workspace-retirement fixture and the historical-only portion of Batch 7 may skip only after this exact bridge is verified; Batch 7's current deterministic Project regressions still run. The bridge is transition-only and must be removed once the next release has a predecessor in public repository history.
