...
const SOURCE_CONTACTS_LIBS = [path.resolve(__dirname, '../../shared/contacts'), path.resolve(__dirname, '../../../shared/contacts')];
const SOURCE_CONTACTS_LIB = SOURCE_CONTACTS_LIBS.find(candidate => fs.existsSync(path.join(candidate, 'core.cjs')));
const INSTALLED_CONTACTS_LIB = path.join(os.homedir(), '.local', 'lib', 'openclaw-contacts');
const CONTACTS_LIB = SOURCE_CONTACTS_LIB ?? INSTALLED_CONTACTS_LIB;
...
module.exports = {
  fs, os, path, crypto, spawnSync, DatabaseSync, contacts, contactStore,
  IMPLEMENTATION_VERSION, SCHEMA_VERSION, TZ, SELF_NAME, DB_PATH, PAYLOAD_LIMIT,
  AppError, die, ok, clock, now, localParts, localDate, localTime, addLocalDays,
  required, optionalString, emojiValue, date, time, deadline, id, projectId,
  recurrenceId, reminderId, calendarDate, localDateAt, localDateTimeAt, localInstant,
  futureReminderInstant, addLocalMonths, dayDiff, weekday, positiveInt, exactKeys,
  normalizeRule, calendarEligible, afterDueDate, payload, stable, hash, ci,
};
