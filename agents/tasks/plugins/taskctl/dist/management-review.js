...
function canonicalBoundary(value) { const d = new Date(value); if (!value.trim() || Number.isNaN(d.getTime()))
    throw new Error("Management Review boundary must be a valid ISO timestamp"); return d.toISOString(); }
function record(value, label) { if (!value || Array.isArray(value) || typeof value !== "object")
...
function nullableText(value, label) { if (value === null)
    return null; if (typeof value !== "string")
    throw new Error(label + " must be a string or null"); return value; }
function parseComment(value) { const r = record(value, "Management Review comment"); return { id: text(r.id, "comment id"), content: text(r.content, "comment content"), createdAt: text(r.created_at, "comment created_at") }; }
function parsePending(value) {
    if (value === null)
        return null;
    const r = record(value, "Management Review pending request");
    if (r.status !== "PENDING")
        throw new Error("Management Review snapshot contains non-pending request");
...
function parseSnapshot(raw, boundary) {
    const root = record(raw, "Management Review snapshot");
    if (root.ok !== true)
        throw new Error("Management Review taskctl snapshot failed");
    ...
    if (!Array.isArray(root.completed) || !Array.isArray(root.not_completed))
        throw new Error("Management Review snapshot populations are missing");
    ...
    const notCompleted = root.not_completed.map(value => { const r = record(value, "non-completed Task"); if (!Array.isArray(r.comments))
        throw new Error("Management Review Task comments are missing"); return { id: text(r.id, "Task id"), title: text(r.title, "Task title"), assignee: text(r.assignee, "assignee"), dueDate: text(r.due_date, "due date"), dueTime: nullableText(r.due_time, "due time"), comments: r.comments.map(parseComment), pending: parsePending(r.pending_deadline_change_request) }; });
    return { boundary, localDate, completed, notCompleted };
}
function compactDate(value, localDate) { const [y, m, d] = value.split("-").map(Number); const cy = Number(localDate.slice(0, 4)); return `${d} ${RU_MONTHS[m - 1]}${y !== cy ? ` ${y}` : ""}`; }
function deadlineText(task, localDate) { const date = compactDate(task.dueDate, localDate); return task.dueTime ? `${date} · ${task.dueTime}` : date; }
function requestedText(request, localDate) { if (!request.requestedDueDate)
    return "без срока"; const date = compactDate(request.requestedDueDate, localDate); return request.requestedDueTime ? `${date} · ${request.requestedDueTime}` : date; }
function fullDate(value) { const [y, m, d] = value.split("-").map(Number); return `${d} ${RU_MONTHS_FULL[m - 1]} ${y}`; }
function groupByAssignee(rows) { const groups = []; for (const row of rows) {
    let group = groups.at(-1);
    if (!group || group.assignee !== row.assignee) {
        group = { assignee: row.assignee, items: [] };
        groups.push(group);
    }
    group.items.push(row);
} return groups; }
function semanticCandidates(tasks) { return tasks.filter(task => !task.pending && task.comments.length > 0); }
function semanticPayload(tasks) { return tasks.map(task => ({ task_id: task.id, title: task.title, assignee: task.assignee, due_date: task.dueDate, due_time: task.dueTime, comments: task.comments.map(c => ({ id: c.id, content: c.content, created_at: c.createdAt })) })); }
function prompt(tasks) {
    return [
...
function chunks(tasks) { const out = []; let cur = []; for (const task of tasks) {
    const next = [...cur, task];
    if (Buffer.byteLength(prompt(next), "utf8") > MODEL_INPUT_LIMIT) {
        if (!cur.length)
            throw new Error("One Management Review task exceeds semantic input bound");
        out.push(cur);
        cur = [task];
    }
    else
    ...
    out.push(cur); if (out.length > MODEL_MAX_CALLS)
    throw new Error("Management Review semantic classification exceeds call bound"); return out; }
function parseSelections(raw, tasks) { let parsed; try {
    parsed = JSON.parse(raw.trim().replace(/^\`\`\`(?:json)?\s*|\s*\`\`\`$/gi, ""));
}
catch {
    throw new Error("Management Review semantic model returned invalid JSON");
} const root = record(parsed, "semantic result"); if (!Array.isArray(root.explanations) || Object.keys(root).some(k => k !== "explanations"))
    throw new Error("Management Review semantic result shape is invalid"); const allowed = new Map(tasks.map(t => [t.id, new Set(t.comments.map(c => c.id))])); const selected = new Map(); for (const value of root.explanations) {
    const x = record(value, "semantic explanation"), taskId = text(x.task_id, "task_id"), commentId = text(x.comment_id, "comment_id");
    if (Object.keys(x).some(k => !["task_id", "comment_id"].includes(k)) || !allowed.get(taskId)?.has(commentId) || selected.has(taskId))
        throw new Error("Management Review semantic result escaped bounded IDs");
    selected.set(taskId, commentId);
} return selected; }
function collectRunText(result) { if (result.meta.aborted || result.meta.error)
    throw new Error("Management Review semantic model failed"); if (result.payloads?.some(p => p.isError))
    throw new Error("Management Review semantic model returned an error payload"); const value = result.meta.finalAssistantVisibleText?.trim() || result.payloads?.filter(p => !p.isReasoning && !p.isCommentary && !p.isError).map(p => p.text?.trim() ?? "").filter(Boolean).join("\n").trim() || ""; if (!value)
    throw new Error("Management Review semantic model returned no text"); return value; }
async function runSemanticModel(api, toolContext, promptText, jobId, signal) {
    const cfg = toolContext.getRuntimeConfig?.() ?? toolContext.runtimeConfig ?? toolContext.config;
    if (!cfg)
        throw new Error("Management Review has no active runtime configuration");
...
async function classifyComments(tasks, runModel, jobId, signal) { const selected = new Map(); for (const batch of chunks(semanticCandidates(tasks))) {
    signal?.throwIfAborted();
    const result = parseSelections(await runModel(prompt(batch), jobId, signal), batch);
    for (const [taskId, commentId] of result)
        selected.set(taskId, commentId);
} return selected; }
function render(snapshot, selections) {
    ...
    const notCompletedBody = groupByAssignee(snapshot.notCompleted).map(group => {
        const items = group.items.map((task, i) => {
            ...
            if (task.pending) {
                lines.push(`   Запрос: перенос до ${requestedText(task.pending, snapshot.localDate)}`);
                lines.push(`   Причина: ${task.pending.reason}`);
            }
            else {
                const commentId = selections.get(task.id), comment = commentId ? task.comments.find(c => c.id === commentId) : undefined;
                if (comment)
                    lines.push(`   Комментарий: ${comment.content}`);
                else
                    lines.push("   ⚠️ Причина не указана");
            }
            return lines.join("\n");
        });
...
export async function executeManagementReview(params, deps = {}) {
    ...
    let selections = new Map(), semanticCalls = 0;
    if (candidates.length) {
        const jobId = deps.toolContext ? parseCurrentCronJobId(deps.toolContext) : "test";
        if (!jobId)
            throw new Error("Management Review semantic classification requires a cron job context");
        ...
        if (!runner)
            throw new Error("Management Review semantic classifier is unavailable");
        const batches = chunks(candidates);
        semanticCalls = batches.length;
        for (const batch of batches) {
            const result = parseSelections(await runner(prompt(batch), jobId, deps.signal), batch);
            for (const [taskId, commentId] of result)
                selections.set(taskId, commentId);
        }
    }
