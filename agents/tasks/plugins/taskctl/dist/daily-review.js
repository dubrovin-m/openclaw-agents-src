...
export function buildDailyReviewScript(params) {
...
function expectedDeclaration(declarationKey) {
...
function assertJobDefinition(params) {
    ...
    for (const [label, job] of [["current", params.current], ["peer", params.peer]]) {
    ...
    const normalizedParams = {
        ...params.groupParams,
        bootstrap_checkpoint: parseIsoInstant(params.groupParams.bootstrap_checkpoint, "bootstrap_checkpoint").iso,
    };
...
function isSuccessfulDeliveredRun(entry) {
...
async function readHistory(params) {
    ...
    for (let page = 0; page < HISTORY_MAX_PAGES; page += 1) {
        ...
        const raw = assertRecord(await params.readGateway("cron.runs", { id: params.jobId, limit: HISTORY_PAGE_SIZE, offset, sortDir: "desc" }, params.signal), "cron.runs result");
        if (!Array.isArray(raw.entries)) {
            throw new Error("cron.runs result has no entries array");
        }
        const pageEntries = raw.entries.map((entry) => assertRecord(entry, "cron run entry"));
        entries.push(...pageEntries);
        const oldest = pageEntries
            .map((entry) => entry.runAtMs)
            .filter((value) => typeof value === "number" && Number.isFinite(value))
            .reduce((min, value) => (min === undefined || value < min ? value : min), undefined);
    ...
    throw new Error(`Daily Review history exceeded the bounded ${HISTORY_MAX_PAGES * HISTORY_PAGE_SIZE}-run scan`);
}
function resolveCheckpoint(params) {
    const delivered = [...params.currentHistory, ...params.peerHistory]
        .filter(isSuccessfulDeliveredRun)
        .filter((entry) => entry.runAtMs < params.currentBoundaryMs && entry.runAtMs >= params.bootstrapMs)
        .sort((a, b) => b.runAtMs - a.runAtMs);
    return delivered[0]?.runAtMs ?? params.bootstrapMs;
}
function sameOccurrenceAlreadyDelivered(history, boundaryMs) {
    const occurrenceDate = moscowParts(boundaryMs).date;
    return history
        .filter(isSuccessfulDeliveredRun)
        .some((entry) => entry.runAtMs < boundaryMs && moscowParts(entry.runAtMs).date === occurrenceDate);
}
function assertNoEarlierPeerRun(params) {
    ...
    if (peerRunningAt < params.boundaryMs ||
...
function nullableSnapshotString(value, field) {
    if (value === null)
        return null;
    if (typeof value !== "string")
        throw new Error(`Daily Review snapshot ${field} must be a string or null`);
    return value;
}
function parseSnapshotTask(value) {
    ...
    if (!taskMatch)
        throw new Error("Daily Review snapshot contains an invalid Task id");
    if (typeof row.title !== "string" || !row.title.trim())
        throw new Error(`Daily Review snapshot ${taskId} has no title`);
    if (typeof row.assignee !== "string" || !row.assignee.trim())
        throw new Error(`Daily Review snapshot ${taskId} has no canonical assignee`);
    if (typeof row.created_at !== "string" || !Number.isFinite(Date.parse(row.created_at)))
        throw new Error(`Daily Review snapshot ${taskId} has invalid created_at`);
    if (!Array.isArray(row.labels))
        throw new Error(`Daily Review snapshot ${taskId} has no labels array`);
    const labels = row.labels.map((value) => {
        const label = assertRecord(value, `Daily Review snapshot ${taskId} Label`);
        if (typeof label.id !== "string" || !/^L-[1-9]\d*$/.test(label.id))
            throw new Error(`Daily Review snapshot ${taskId} contains an invalid Label id`);
        if (typeof label.display_name !== "string" || !label.display_name.trim())
            throw new Error(`Daily Review snapshot ${taskId} contains an invalid Label name`);
        if (label.emoji !== null && typeof label.emoji !== "string")
            throw new Error(`Daily Review snapshot ${taskId} contains invalid Label emoji`);
        return { id: label.id, displayName: label.display_name, emoji: label.emoji };
    });
...
async function loadReviewSnapshot(boundaryIso, signal) {
    ...
    if (result.ok !== true) {
        const error = result.error && typeof result.error === "object" && !Array.isArray(result.error) ? result.error : null;
        throw new Error(`Daily Review taskctl snapshot failed: ${typeof error?.message === "string" ? error.message : "unknown error"}`);
    }
    if (result.schema_version !== TASKCTL_SCHEMA_VERSION)
        throw new Error(`Daily Review taskctl snapshot schema mismatch: ${String(result.schema_version)}`);
    if (result.boundary !== boundaryIso)
        throw new Error("Daily Review taskctl snapshot boundary mismatch");
    ...
    if (!Array.isArray(result.tasks))
        throw new Error("Daily Review taskctl snapshot has no tasks array");
...
function boundedChunks(items, maxBytes) {
    const chunks = [];
    let current = [];
    let currentBytes = 2;
...
function semanticPayload(candidates, population) {
    return {
        new_candidates: candidates.map(semanticTask),
        open_population: population.map(semanticTask),
    };
}
function semanticPrompt(candidates, population) {
    return [
...
function semanticRetryPrompt(candidates, population) {
...
function parseSemanticPairs(text, candidateIds, populationIds) {
    let raw = text.trim();
    ...
    for (const pair of root.pairs) {
        ...
        const [firstId, secondId] = [candidateId, otherId].sort((a, b) => numericTaskId(a) - numericTaskId(b));
...
function collectRunText(result) {
    ...
    const text = result.meta.finalAssistantVisibleText?.trim() ||
        result.payloads
...
async function runSemanticModel(api, toolContext, prompt, jobId, signal) {
...
function assertSemanticCallCapacity(estimatedCalls) {
    const worstCaseCalls = estimatedCalls * 2;
    if (worstCaseCalls > MODEL_MAX_CALLS) {
        throw new Error(`Daily Review semantic comparison requires up to ${worstCaseCalls} calls including one retry per batch, exceeding the bounded limit ${MODEL_MAX_CALLS}`);
    }
}
async function findDuplicatePairs(params) {
    if (params.candidates.length === 0) {
        return { pairs: [], calls: 0 };
    }
    const candidateChunks = boundedChunks(params.candidates, Math.floor(MODEL_INPUT_TARGET_BYTES * 0.35));
    const populationChunks = boundedChunks(params.population, Math.floor(MODEL_INPUT_TARGET_BYTES * 0.65));
    ...
    return {
        pairs: [...pairMap.values()].sort((a, b) => numericTaskId(a.firstId) - numericTaskId(b.firstId) ||
...
function renderDuplicateWarnings(pairs, byId) {
    if (pairs.length === 0)
        return "";
    return [
        "Возможные дубли",
...
function reviewInboxCount(declarationKey, inboxCount) {
    return declarationKey === DAILY_REVIEW_DECLARATIONS.morning ? inboxCount : undefined;
}
function renderReview(tasks, pairs, boundaryMs, inboxCount) {
...
async function runGatewayRead(api, method, params, signal) {
...
export async function executeDailyReview(params, deps) {
    ...
    const readGateway = deps.readGateway ??
        ((method, gatewayParams, signal) => runGatewayRead(deps.api, method, gatewayParams, signal));
    ...
    const snapshotLoader = deps.loadSnapshot ?? ((value) => loadReviewSnapshot(value, deps.signal));
    ...
    const runModel = deps.runSemanticModel ??
        ((prompt, jobId, signal) => runSemanticModel(deps.api, deps.toolContext, prompt, jobId, signal));
