...
const validNewTask = (v) => { if (v === null || typeof v !== "object" || Array.isArray(v))
    return false; const x = v, allowed = ["title", "assignee", "assignee_id", "create_assignee", "status", "due_date", "due_time", "labels", "project_id"]; if (Object.keys(x).some(k => !allowed.includes(k)) || !validString(x.title, 2000))
    return false; const assignees = ["assignee", "assignee_id"].filter(k => has(x, k)); if (assignees.length !== 1)
    return false; if (has(x, "assignee") && !validString(x.assignee, 500) || has(x, "assignee_id") && !validId(x.assignee_id, "P"))
    return false; if (has(x, "create_assignee") && (typeof x.create_assignee !== "boolean" || !has(x, "assignee")))
...
function appendChunk(current, chunk, state, limit) { const b = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk); state.bytes += b.byteLength; if (state.bytes <= limit)
    current.push(b); return state.bytes <= limit; }
export async function executeTaskctl(action, payload, options = {}) { const v = validateAndSanitizePayload(action, payload); if (!v.ok)
    return v.error; return runTaskctl(v.action, v.payload, options); }
async function runInvocation(inv, options = {}) {
    const timeoutMs = options.timeoutMs ?? TASKCTL_TIMEOUT_MS, limit = options.outputLimitBytes ?? TASKCTL_OUTPUT_LIMIT_BYTES, spawnImpl = options.spawnImpl ?? spawn;
    const outcome = await new Promise(resolve => { let child; try {
        child = spawnImpl(inv.executable, inv.argv, inv.options);
    }
    catch (error) {
        resolve({ exitCode: null, signal: null, stdout: "", stderr: "", timedOut: false, aborted: false, outputExceeded: false, spawnError: error instanceof Error ? error.message : String(error) });
        return;
    } const stdout = [], stderr = []; const so = { bytes: 0 }, se = { bytes: 0 }; let timedOut = false, aborted = false, outputExceeded = false, settled = false; const kill = () => { if (!child.killed)
        child.kill("SIGKILL"); }; const timer = setTimeout(() => { timedOut = true; kill(); }, timeoutMs); timer.unref?.(); const onAbort = () => { aborted = true; kill(); }; if (options.signal?.aborted)
        onAbort();
    else
        options.signal?.addEventListener("abort", onAbort, { once: true }); child.stdout.on("data", c => { if (!appendChunk(stdout, c, so, limit)) {
        outputExceeded = true;
        kill();
    } }); child.stderr.on("data", c => { if (!appendChunk(stderr, c, se, limit)) {
        outputExceeded = true;
        kill();
    } }); const finish = (exitCode, signal, spawnError) => { if (settled)
        return; settled = true; clearTimeout(timer); options.signal?.removeEventListener("abort", onAbort); resolve({ exitCode, signal, stdout: Buffer.concat(stdout).toString("utf8"), stderr: Buffer.concat(stderr).toString("utf8"), timedOut, aborted, outputExceeded, ...(spawnError ? { spawnError } : {}) }); }; child.on("error", e => finish(null, null, e.message)); child.on("close", (c, s) => finish(c, s)); });
    if (outcome.spawnError)
        return structuredError("TASKCTL_SPAWN_ERROR", "Failed to start taskctl", { detail: outcome.spawnError });
    if (outcome.aborted)
        return structuredError("TASKCTL_ABORTED", "taskctl call was aborted");
    if (outcome.timedOut)
        return structuredError("TASKCTL_TIMEOUT", "taskctl call timed out", { timeout_ms: timeoutMs });
    if (outcome.outputExceeded)
        return structuredError("TASKCTL_OUTPUT_LIMIT", "taskctl output exceeded the configured limit", { output_limit_bytes: limit });
    let parsed;
    try {
        parsed = JSON.parse(outcome.stdout);
    }
    catch {
        return structuredError("TASKCTL_INVALID_JSON", "taskctl stdout was not valid JSON", { exit_code: outcome.exitCode, signal: outcome.signal, stderr: outcome.stderr.slice(0, 4096) });
    }
    if (outcome.exitCode !== 0 || outcome.signal !== null || outcome.stderr.length > 0)
        return structuredError("TASKCTL_PROCESS_ERROR", "taskctl reported a process error", { exit_code: outcome.exitCode, signal: outcome.signal, stderr: outcome.stderr.slice(0, 4096), taskctl: parsed });
    return parsed;
}
export async function runTaskctl(action, payload, options = {}) { return runInvocation(buildInvocation(action, payload), options); }
export function buildDailyReviewSnapshotInvocation(boundaryIso) {
    if (typeof boundaryIso !== "string" || !boundaryIso.trim())
        throw new Error("Daily Review snapshot boundary is required");
    const instant = new Date(boundaryIso);
    if (Number.isNaN(instant.getTime()))
        throw new Error("Daily Review snapshot boundary must be a valid ISO timestamp");
