...
const validNewTask=(v:unknown)=>{if(v===null||typeof v!=="object"||Array.isArray(v))return false;const x=v as JsonObject,allowed=["title","assignee","assignee_id","create_assignee","status","due_date","due_time","labels","project_id"];if(Object.keys(x).some(k=>!allowed.includes(k))||!validString(x.title,2000))return false;const assignees=["assignee","assignee_id"].filter(k=>has(x,k));if(assignees.length!==1)return false;if(has(x,"assignee")&&!validString(x.assignee,500)||has(x,"assignee_id")&&!validId(x.assignee_id,"P"))return false;if(has(x,"create_assignee")&&(typeof x.create_assignee!=="boolean"||!has(x,"assignee")))return false;if(has(x,"status")&&!["OPEN","DONE"].includes(String(x.status)))return false;if(has(x,"due_date")&&!validDate(x.due_date)||has(x,"due_time")&&!validTime(x.due_time))return false;if(x.due_time!==undefined&&x.due_time!==null&&(!has(x,"due_date")||x.due_date===null))return false;if(has(x,"labels")&&(!Array.isArray(x.labels)||x.labels.length>20||!x.labels.every(validLabelSpec)))return false;if(has(x,"project_id")&&!validProjectId(x.project_id))return false;return true;};
