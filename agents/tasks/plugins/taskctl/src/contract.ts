...
const entityId = (prefix: "I" | "T" | "P" | "L" | "PRJ") => Type.Union([
...
const FIELD_SCHEMAS: Record<string, TSchema> = {
...
export const TASKCTL_ACTIONS = Object.freeze(Object.keys(ACTION_REGISTRY) as TaskctlAction[]);

export function getActionDefinition(action: TaskctlAction): ActionDefinition {
  return ACTION_REGISTRY[action] as ActionDefinition;
}

function idSchemaForAction(action: TaskctlAction): TSchema {
  if (action.startsWith("inbox_")) return inboxId;
...
function fieldSchema(action: TaskctlAction, field: string): TSchema {
  if (field === "id") return idSchemaForAction(action);
  if (field === "from_id" || field === "into_id") return action.startsWith("person_") ? personId : labelId;
  if (field === "status") {
    if (action === "task_create") return taskStatus;
    if (action === "project_list") return projectFilterStatus;
    if (action === "recurrence_list") return recurrenceFilterStatus;
    return filterStatus;
  }
...
export function actionPayloadSchema(action: TaskctlAction): JsonSchema {
...
export function actionToolParameters(action: TaskctlAction) {
  return Type.Unsafe<Record<string, unknown>>(actionPayloadSchema(action));
}
