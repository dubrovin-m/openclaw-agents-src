...
function firstLabel(task: ReviewTask) {
...
function comparePrimary(
  ...
  if (Boolean(aLabel) !== Boolean(bLabel)) return aLabel ? -1 : 1;
  if (aLabel && bLabel) {
    ...
    if (byLabelId !== 0) return byLabelId;
  }
...
function numericLabelId(id: string): number {
...
function renderDeadline(task: ReviewTask, localDate: string) {
...
function renderTask(task: ReviewTask, localDate: string) {
...
function renderPrimary(tasks: ReviewTask[], boundaryMs: number) {
  ...
  const overdue = sorted.filter(
    (task) => primaryBucket(task, local.date, local.time) === 0,
  );
