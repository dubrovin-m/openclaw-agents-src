...
function canonicalBoundary(value: string): string {
...
export async function executeManagementReview(
  params: ManagementReviewParams,
  deps: { signal?: AbortSignal; runSnapshot?: typeof runManagementReviewSnapshot } = {},
) {
  ...
  if (record.boundary !== boundary) throw new Error("Management Review snapshot boundary mismatch");
...
export function createManagementReviewTool(toolContext: OpenClawPluginToolContext): AnyAgentTool | null {
  ...
  return {
    ...
    execute: async (_toolCallId, params, signal) => {
      const result = await executeManagementReview(params as ManagementReviewParams, { signal });
      return {
