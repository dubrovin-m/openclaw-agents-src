...
for (const required of ["const IMPLEMENTATION_VERSION = '0.4.14';", 'function openReadDb()', 'readOnly:true', 'PRAGMA query_only=ON', "scope==='review'&&action==='snapshot'", "scope==='review'&&action==='management-snapshot'"]) {
...
if (result.schema_version !== 9 || result.implementation_version !== '0.4.14') throw new Error(`Batch 8 runtime initialized ${result.implementation_version} schema ${result.schema_version}, expected taskctl 0.4.14 / schema 9`);
NODE
...
node - "$snapshot" <<'NODE'
const result = JSON.parse(process.argv[2]);
if (!result.ok || result.implementation_version !== '0.4.14' || result.schema_version !== 9 || result.boundary !== '2026-09-05T06:30:00.000Z') process.exit(2);
