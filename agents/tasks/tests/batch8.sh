...
node - "$ROOT" "$EXPECTED_TASKCTL_VERSION" <<'NODE'
const fs = require('node:fs');
const path = require('node:path');
const root = process.argv[2];
const expectedTaskctlVersion = process.argv[3];
...
node - "$init" "$EXPECTED_TASKCTL_VERSION" <<'NODE'
const result = JSON.parse(process.argv[2]), expected = process.argv[3];
if (result.schema_version !== 9 || result.implementation_version !== expected) throw new Error(`Batch 8 runtime initialized ${result.implementation_version} schema ${result.schema_version}, expected taskctl ${expected} / schema 9`);
NODE
...
node - "$snapshot" "$EXPECTED_TASKCTL_VERSION" <<'NODE'
const result = JSON.parse(process.argv[2]), expected = process.argv[3];
if (!result.ok || result.implementation_version !== expected || result.schema_version !== 9 || result.boundary !== '2026-09-05T06:30:00.000Z') process.exit(2);
