...
# This is a frozen historical migration/recovery qualification. Keep its
# OpenClaw contract bound to the predecessor generation rather than the
# repository's current rollout target.
...
set +e
CONTACTCTL_TEST_MIGRATION_FAULT=after-important-date-ddl HOME="$R/home" bash "$ROOT/deploy.sh" --test-root "$R" --apply >/dev/null 2>&1
CODE=$?
set -e
[ "$CODE" -eq 1 ] || fail "faulted Contacts migration did not roll back"
assert_predecessor "$R"
