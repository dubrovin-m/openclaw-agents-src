...
const release=require(process.argv[2]);process.stdout.write([release.generation.taskctl_version,release.generation.sqlite_schema].join(' '));
NODE
)"
node -e 'const x=JSON.parse(process.argv[1]);if(x.ok!==true||x.implementation_version!==process.argv[2]||x.schema_version!==Number(process.argv[3]))process.exit(1)' "$DEFAULT_HEALTH" "$TARGET_TASKCTL" "$TARGET_SCHEMA" || fail "default taskctl path does not resolve current isolated store"
