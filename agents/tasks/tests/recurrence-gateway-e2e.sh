...
node -e 'const x=JSON.parse(process.argv[1]);if(x.ok!==true||x.implementation_version!=="0.4.7"||x.schema_version!==6)process.exit(1)' "$DEFAULT_HEALTH" || fail "default taskctl path does not resolve isolated schema 6 store"
