...
import {
  CONTACT_DATE_REMINDER_DISPATCH_TOOL,
  createImportantDateDispatchTool,
  importantDateDispatchParameters,
  registerImportantDateRuntime,
...
const entry=defineToolPlugin({
  id:"contacts",name:"Contacts",description:"Bounded shared Person identity and Important Dates operations.",
  tools:(tool)=>[
    ...(Object.keys(ACTIONS) as ContactAction[]).map(action=>tool({
      name:action,label:meta[action].label,description:meta[action].description,parameters:CONTACT_SCHEMAS[action],
      factory:()=>({name:action,label:meta[action].label,description:meta[action].description,parameters:CONTACT_SCHEMAS[action],catalogMode:"direct-only",execute:async(_toolCallId,params,signal)=>jsonResult(await executeContactctl(action,params as Record<string,unknown>,{signal}))}),
    })),
    tool({
